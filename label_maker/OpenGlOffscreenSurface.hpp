...
class OpenGlOffscreenSurface: public QOffscreenSurface,protected QOpenGLFunctions
{
    Q_OBJECT
signals:
...
public:
    ...
    inline QSize get_size(){return m_size;}

    inline void ini(){ create();  // Some platforms require this function to be called on the main (GUI) thread
                       initializeInternal();}
    void set_size(int height,int width){_h=height,_w=width;}
public slots:
...
private:
    ...
    QSize m_size;
    QImage img;
    bool isini=false,isresized=false;
    int _h,_w;
};
